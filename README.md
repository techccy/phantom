...
- 目标区域内粒子施加**共同位移向量** → 相干运动显影
- 反密度分析：亮度随机闪烁 + 密度扰动，防像素密度抠图
...
浏览器访问 `http://localhost:5173`，按住"按住跟随方块"按钮，用肉眼跟随移动方块移动约 3 秒后松开，即得到判定。
...
| `PHANTOM_MAX_DRIFT_SECONDS` | `3.0` | 提交时效上限（手册 §四.2） |
...
用户按住激活键跟随方块 3s → tracker 采集轨迹
前端 AES-GCM 加密轨迹 ──▶ /verify（立即提交，3s 时效）
